...
import { fetchProjects } from '../../data/service.js';
import { overviewKpis, projectsNeedingAttention, coverageTrend, singleOwnerSystems, engineeringChain } from '../../data/service.js';
 ...
 */
const Overview = () => {
  const { loading } = useData(fetchProjects);
  const kpis = overviewKpis();
  const trend = coverageTrend();
  const attention = projectsNeedingAttention(5);
  const singleOwners = singleOwnerSystems();
  const chain = engineeringChain(attention[0]?.id);
  ...
  return (
    <Box>
      ...
      {loading ? (
        <LoadingState variant="grid" sx={{ mt: 3 }} />
      ) : (
